struct Treap {
	...
	struct Node {
		...
		const uint32_t h;
		Data data;
		Node(uint32_t h): lchild(0), rchild(0), sub(1), h(h), data() {}
	};
	size_t root;
	vector<uint32_t> h;
	vector<Node> nodes;
	...
	Treap(const size_t size=0): root(0), h(random_permutation(size)) {
		nodes.reserve(size+1);
		new_node();
	}
	size_t cnt_left(const Node& t) const { return t.lchild==0?0:(size_t)nodes[t.lchild].sub; }
	size_t cnt_right(const Node& t) const { return t.rchild==0?0:(size_t)nodes[t.rchild].sub; }
	...
	void merge(size_t tl, size_t tr, size_t& dest) {
		if(tl==0||tr==0) { dest = tl+tr; return; }
	...
	int64_t& find(size_t index) {
		if(root == 0 || nodes[root].sub >= index) return nullptr;
