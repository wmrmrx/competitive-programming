struct SuffixArray {
	vector<int> p, arr, lcp;

	template <typename T>
	SuffixArray(int sz, const T s[]): p(sz+1), arr(sz+1), lcp(sz) {
		iota(p.begin()+1, p.end(), 0);
		sort(p.begin()+1, p.end(), [&](int i, int j) { return s[i] < s[j]; });
		p[0] = sz++;
		int alph = *max_element(s, s+sz) - *min_element(s+sz) + 1;
		vector<int> ra(sz), newra(sz), newp(sz), cnt(2+max(sz, alph));
		ra[p[1]] = 1;
